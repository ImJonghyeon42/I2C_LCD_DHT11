...
//----------DEBUGINH WIRE--------------------
	...
	debounce ok_debouncer (
		.clk(clk_1MHz),
		.raw_btn(virtual_button),      // 버튼이 눌렸을 때 1이 되도록 신호 반전
		.rst_n(rst_n),
		.clean_btn(joy_ok_clean)
	);
	...
	assign led_bar[2:0] = led_bar_w[4:2];
	assign led_bar[3] = 1'b0;
	assign led_bar[4] = joy_ok_clean;
