...
//---------UART WIRE----------------
	...
	reg 			rx_read_en;
//----------DEBUG WIRE-----------------
	wire [2:0] uart_debug_state;
//----------CDC(Clock Domain Crossing WIRE-----------	
	reg			uart_rx_valid_synced;
	reg				uart_rx_read_en_sync1, uart_rx_read_en_sync2;
	...
	uart_receiver uart_inst(
        ...
        .reset      (rst_n),
        .rxd        (uart_rxd),
        .rx_read_en (uart_rx_read_en_sync2),
	...
	mode_controller mode_ctrl (
		...
		.uart_data_valid(uart_rx_valid_synced),
    ...
    // -- CDC Logic --
    // Synchronize read_en from 1MHz (slow) to 100MHz (fast) domain
    always @(posedge clk or negedge rst_n) begin
        if(~rst_n) {uart_rx_read_en_sync1, uart_rx_read_en_sync2} <= 2'b0;
        else {uart_rx_read_en_sync1, uart_rx_read_en_sync2} <= {rx_read_en, uart_rx_read_en_sync1};
    end

    // Synchronize valid from 100MHz (fast) to 1MHz (slow) domain
    reg uart_rx_valid_sync1;
    always @(posedge clk_1MHz or negedge rst_n) begin
        if(~rst_n) {uart_rx_valid_sync1, uart_rx_valid_synced} <= 2'b0;
        else {uart_rx_valid_sync1, uart_rx_valid_synced} <= {rx_valid, uart_rx_valid_sync1};
    end

    // -- UART Receive Logic (generates the read enable signal in 1MHz domain) --
    always @(posedge clk_1MHz or negedge rst_n) begin
        if (~rst_n) rx_read_en <= 1'b0;
        else rx_read_en <= uart_rx_valid_synced;
    end
    
    // -- [DEBUG] Display UART FSM state on LEDs --
    assign led_bar = {5'b0, uart_debug_state};
