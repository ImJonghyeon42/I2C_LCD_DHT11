...
//joystick_to_button 모듈이 오작동하는지, 아니면 애초에 잘못된 x_axis_in, y_axis_in 값이 들어오고 있는지부터 확인
//진단 방법: 디바운서 출력 직접 확인용 led 배치
//sw를 한번 눌러도 led가 하나씩 다 켜지는 문제 발생 -> debounce test
module top(
    input           clk,
    input           rst_n,
    output          scl,
...
//--------BTN WIRE--------------------	
	...
	wire					btn_L_pulse,btn_R_pulse,btn_U_pulse,btn_D_pulse;
	wire 				joystick_up_held;
	wire 				joystick_down_held;
	wire 				joystick_left_held;
	wire					joy_ok_clean;
//----------DHT11 WIRE----------------	
...
//----------DEBUGINH WIRE--------------------
	...
	wire	[4:0]	led_bar_w;
	reg [21:0] test_counter = 0;
	
	always @(posedge clk_1MHz) test_counter <= test_counter + 1;
	wire virtual_button = (test_counter < 1_000_000);
	...
	joystick_to_button joy_CTR(
		...
		.btn_L_out(btn_L_pulse),
		.btn_R_out(btn_R_pulse),
		.btn_U_out(btn_U_pulse),
		.btn_D_out(btn_D_pulse)
	);
	...
	assign joystick_up_held = (joystick_y_adc > 800);
	assign joystick_down_held = (joystick_y_adc < 200);
	assign joystick_left_held = (joystick_x_adc < 200);
	...
	mode_controller mode_ctrl (
		...
		.btn_R(btn_R_pulse),
		.btn_U(btn_U_pulse),
		.btn_U_level(joystick_up_held),
		.btn_D_level(joystick_down_held),
		.btn_L_level(joystick_left_held),
